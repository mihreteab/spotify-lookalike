...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ playlist }: AudioPlayerProps) => {
  const audioRef = useRef<HTMLAudioElement>(null);
  const [currentPlaying, setCurrentPlaying] = useState<PlaylistType>(playlist.find(play => play.currentplaying)! ?? playlist[0])
  const [isPlaying, setIsPlaying] = useState(false);
  const [currentTime, setCurrentTime] = useState(0);
  const [volume, setVolume] = useState(0.5);
  const [duration, setDuration] = useState(0);
  const [open, setOpen] = useState(false)
  const [like, setLike] = useState(currentPlaying?.liked ?? false)
  ...
  }, [currentPlaying.src, currentPlaying.musicType, currentPlaying.id]);
  ...
  const likeButton = () => {
    switch (currentPlaying.musicType) {
      case 'MOSTLISTEND':
        return <button
          className='absolute bottom-0 top-0 right-0 left-0'
          onClick={() => {
            likeMostListend(currentPlaying.id)
            setLike(!like)
          }}
        >
        </button>
      case 'PLAYLISTFORYOU':
        return <button
          className='absolute bottom-0 top-0 right-0 left-0'
          onClick={() => {
            likePlaylistForYou(currentPlaying.id)
            setLike(!like)
          }}
        ></button>
      case 'TRACK_FOR_YOU':
        return <button
          className='absolute bottom-0 top-0 right-0 left-0'
          onClick={() => {
            likeTrackOfTheWeek(currentPlaying.id)
            setLike(!like)
          }
          }
        ></button>
  ...
  useEffect(() => {
    if (audioRef.current) {
      audioRef.current.pause();
      audioRef.current.src = currentPlaying.src;
      audioRef.current.load();
      audioRef.current.play();
      setIsPlaying(true);
      setCurrentTime(0);
    }
  }, [currentPlaying.src, currentPlaying.id, currentPlaying]);
  ...
  const playNext = () => {
    const currentIndex = playlist.indexOf(currentPlaying);
    const nextIndex = (currentIndex + 1) % playlist.length;
    setCurrentPlaying(playlist[nextIndex]);
  };

  const playPrev = () => {
    const currentIndex = playlist.indexOf(currentPlaying);
    const prevIndex = (currentIndex - 1 + playlist.length) % playlist.length;
    setCurrentPlaying(playlist[prevIndex]);
  };

  useEffect(() => {
    setCurrentPlaying(playlist.find(play => play.currentplaying)! ?? playlist[0])
  }, [playlist])

  return (
    <div>
      <div className="bg-[#333333]  flex flex-col gap-2 fixed bottom-16 md:bottom-0 left-0 w-full text-whit px-4 pt-3 rounded-lg pb-2 md:pt-8 md:flex-row md:items-center">
        <div className='flex justify-between items-center'>
          <div
            onClick={openFullScreen}
            className='flex gap-2 shrink-0 items-center lg:pr-24'>
            <Image
              src={currentPlaying.poster}
            ...
            <div className='md:hidden lg:block'>
              <p className='text-xs'>{currentPlaying.title}</p>
              <p className='text-xs text-[#9CA3AF]'>{currentPlaying.album}</p>
            </div>
          </div>
          <div className='flex'>
        ...
        <audio ref={audioRef} src={currentPlaying.src} className="hidden" autoPlay>
      ...
      <div className={classNames('flex flex-col gap-5 fixed p-5 border-red bg-[#000000] bottom-16 left-0 right-0', {
        ...
        <div className='flex justify-between items-center'>
          ...
          <div className='relative'>
            {likeButton()}
            {
              like ? <svg width="25" height="22" viewBox="0 0 25 22" fill="#2BD268" xmlns="http://www.w3.org/2000/svg">
                <path d="M1.42818 5.07229C1.71087 4.38983 2.12521 3.76973 2.64755 3.2474C3.16988 2.72507 3.78998 2.31072 4.47244 2.02804C5.1549 1.74535 5.88636 1.59985 6.62505 1.59985C7.36374 1.59985 8.0952 1.74535 8.77766 2.02804C9.46012 2.31072 10.0802 2.72507 10.6025 3.2474L12.25 4.8949L13.8975 3.2474C14.9524 2.1925 16.3832 1.59987 17.875 1.59987C19.3669 1.59987 20.7976 2.1925 21.8525 3.2474C22.9074 4.3023 23.5001 5.73305 23.5001 7.2249C23.5001 8.71675 22.9074 10.1475 21.8525 11.2024L12.25 20.8049L2.64755 11.2024C2.12521 10.6801 1.71087 10.06 1.42818 9.37751C1.1455 8.69505 1 7.96359 1 7.2249C1 6.48621 1.1455 5.75475 1.42818 5.07229Z" stroke="white" stroke-width="0" stroke-linecap="round" stroke-linejoin="round" />
              </svg> :
        ...
        <div>
          ...
          <div className='flex justify-between'>
            <span className="text-sm md:block">{formatTime(currentTime)}</span>
            <span className="text-sm md:block">{formatTime(duration)}</span>
          </div>
          <div>
